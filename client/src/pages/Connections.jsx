    ...
    return (
        <div className="min-h-screen bg-slate-50">
            <div className="max-w-6xl mx-auto p-6">
                ...
                {/* Tabs */}
                <div className="inline-flex flex-wrap items-ceter border border-gray-200 rounded-md p-1 bg-white shadow-sm">
                    {
                        dataArray.map((tab)=>(
                            <button 
                ...
                <div className="flex flex-wrap gap-6 mt-6">
                    {dataArray.find((item)=>item.label === currentTab).value.map((user)=>(
                        <div key={user._id} className="w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md">
                            <img src={user.profile_picture} alt="the profile picture of the user that you connect or may connect with" className="rounded-full w-12 h-12 shadow-md mx-auto"/>
